...
DATASET_PATH = "./merged_pii_dataset"
MAX_SEQ_LENGTH = 256
OUTPUT_DIR = "./nemotronpiidetect-deberta-base"

def get_unique_labels(dataset, num_samples=1000000):
...
def main():
    print(f"[1/5] Loading local dataset from {DATASET_PATH}...")
    dataset = load_from_disk(DATASET_PATH)
    
    # We are training on the full dataset, no subsetting!
    print(f"      Train size: {len(dataset['train'])} records")
    print(f"      Test size: {len(dataset['test'])} records")
    ...
    tokenized_train = dataset["train"].map(
        tokenize_fn, batched=True, num_proc=8, remove_columns=dataset["train"].column_names, desc="Tokenizing Train"
    )
    tokenized_eval = dataset["test"].map(
        tokenize_fn, batched=True, num_proc=8, remove_columns=dataset["test"].column_names, desc="Tokenizing Test"
    )
    ...
    # -------------------------------------------------------------
    # OPTIMIZATIONS FOR NVIDIA A40 (48GB VRAM)
    # -------------------------------------------------------------
    training_args = TrainingArguments(
        output_dir=OUTPUT_DIR,
        learning_rate=3e-5,
        
        # Maximize GPU throughput with large batches
        ...
        # A40 supports Mixed Precision (bf16) which is extremely fast and stable
        bf16=True,
        
        # Standard configs for extensive 10h training logic
        num_train_epochs=3, # Consider increasing if 10hrs is your budget
        weight_decay=0.01,
        eval_strategy="epoch",
        save_strategy="epoch",
    ...
    print("\n[5/5] Starting full training on A40 GPU...")
    trainer.train()
