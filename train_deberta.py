...
def main():
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=4,     # Speed Opt: Small model + seqlen 128 allows larger physical batch
        gradient_accumulation_steps=2,     # VRAM Opt: Keeps effective batch size at 8 (4 * 2)
        per_device_eval_batch_size=16,     # Speed Opt: Eval has no gradients, can use a larger batch size
        ...
        gradient_checkpointing=False,      # Speed Opt: Disabled to remove the ~30% compute overhead (fits in 4GB now)
